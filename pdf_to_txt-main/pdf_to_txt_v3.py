...
def change_pdf_to_txt(name):
    ...
    for i in range(len(pdf.pages)):
        ...
        if len(tables) >= 1:
            ...
            for _t in range(len(text_list)):
                ...
                all_text[allrow]['inside'] = text_list[_t]
                allrow = allrow + 1
    save_path_1 = 'D:\LLM_dev\FinGPT-intern\pdf_to_txt-main\\txt\\' + name.split('\\')[-1].replace('.pdf', '.txt')
    save_path_2 = 'D:\LLM_dev\FinGPT-intern\pdf_to_txt-main\\txt\\' + name.split('\\')[-1].replace('.pdf', '_txt.txt')
    for key in all_text.keys():
        ...
        with open(save_path_2, 'a+', encoding='utf-8') as file:
            file.write(json.dumps(all_text[key]['inside'],ensure_ascii=False) + '\n')
