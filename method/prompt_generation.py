...
def search_db(stock_name, q):
    ...
    for key, value in dict.items():
        if stock_name in value:
            # found += 1
            path = os.path.join(current_dir + '\\vector_store', str(key))
            if cp_db == None:  # 第一个向量数据库
                try:
                ...
                try:
                    ...
                    pass
    masked_question = q.replace(stock_name, "公司")  # 隐去公司简称
    if cp_db != None:
        contexts = cp_db.similarity_search(masked_question)
        count = 0
        for context in contexts:
            if count < 3:
                knowledge = knowledge + '\n' + context.page_content[:300]
                count += 1
            else:
                break
    return knowledge
...
class GLMPrompt:
    ...
    def handler_q(self, q):
        ...
        if has_q:
            ...
            prompt_res = [prompt for prompt in prompts if len(prompt) > 5 and '0.00' not in prompt]
            if prompt_res:
                prompt_res = '\n'.join(prompt_res)
                ...
                for keywords, (template, data_func) in keyword_mappings.items():
                    if any(keyword in q for keyword in keywords) and contains_year:
                        ...
                        if data is None:
                            # 加入向量数据库top3信息
                            data = search_db(stock_name, q)
                            return template_manager.get_template("basic_input").format(context=f"{stock_info}\n{data}", question=q)
                        else:
